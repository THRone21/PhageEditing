for i in $(ls /ldfssz1/ST_INFECTION/P17Z10200N0246_Phage_XMF/USER/thr/BESphage/*.fa)
	do
		A=`echo $(basename $i .fa)`
		##目的路径 防止任务跑断 故判断续跑检查用
		address="/ldfssz1/ST_INFECTION/P17Z10200N0246_Phage_XMF/USER/songwenchen/Kp_phage_2019/new_asm_fasta/annotation/$A"
		if [ -d $address ];
		then
...
cd $address
mkdir ./01.prodigal
/hwfssz5/ST_INFECTION/Phage/P17Z10200N0246_Phage_XMF/share/02.annotation/software/Prodigal/prodigal -i $i -o "./01.prodigal/$A\_genome.prodigal.gff" -a "./01.prodigal/$A\_genome.proteins.faa" -d "./01.prodigal/$A\_genome.genes.fasta" -f gff -c
mkdir ./02.blastp
